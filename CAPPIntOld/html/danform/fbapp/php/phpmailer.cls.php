...
/*
 * Replaced all 'echo' statement by 'writeErrorLog'
 * cdg@coffeecup.com, Sept 2011.
 */
 ...
 /**
  ...
  private function AddAnAddress($kind, $address, $name = '') {
    if (!preg_match('/^(to|cc|bcc|ReplyTo)$/', $kind)) {
      writeErrorLog( 'Invalid recipient array:', $kind );
      return false;
    }
    ...
    if (!self::ValidateAddress($address)) {
      ...
      writeErrorLog( $this->Lang('invalid_address').': '.$address );
      return false;
    }
    if ($kind != 'ReplyTo') {
 ...
 */
  ...
  protected function SmtpSend($header, $body) {
    require_once 'fbapp/php/class.smtp.php';
    $bad_rcpt = array();
